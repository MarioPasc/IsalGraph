...
Computes the optimal alignment between two IsalGraph strings and renders
it as a colour-coded two-row character grid. This is the primitive behind
the graph-distance figures: ``Levenshtein(w*_G, w*_H)`` approximates the
graph edit distance, and the alignment shows *where* two encodings
diverge.

Absorbed from ``benchmarks/real_data/eval_visualizations/string_alignment.py``.
The behaviour is unchanged; the module-scope matplotlib and numpy imports
moved into the function bodies so ``import isalgraph.viz`` stays
dependency-free.
"""
