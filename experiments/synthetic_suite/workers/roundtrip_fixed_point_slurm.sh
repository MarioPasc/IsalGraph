#!/usr/bin/env bash
# SLURM compute worker for roundtrip_fixed_point benchmark
set -euo pipefail

echo "=== Round-trip Fixed Point: SLURM Worker ==="
...
python -m benchmarks.roundtrip_fixed_point.roundtrip_fixed_point \
    --seed "$SEED" \
    --output-dir "$OUT_DIR" \
    --mode picasso \
    --n-workers "$N_WORKERS" \
