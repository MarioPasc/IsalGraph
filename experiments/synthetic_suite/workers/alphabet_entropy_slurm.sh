#!/usr/bin/env bash
# SLURM compute worker for alphabet_entropy benchmark
set -euo pipefail

echo "=== Alphabet Entropy: SLURM Worker ==="
...
python -m benchmarks.alphabet_entropy.alphabet_entropy \
    --seed "$SEED" \
    --output-dir "$OUT_DIR" \
    --mode picasso \
    --n-workers "$N_WORKERS" \
