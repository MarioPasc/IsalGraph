#!/usr/bin/env bash
# SLURM compute worker for starting_node_sensitivity benchmark
set -euo pipefail

echo "=== Starting Node Sensitivity: SLURM Worker ==="
...
CONFIG="${REPO_DIR}/experiments/synthetic_suite/config.yaml"
cd "$REPO_DIR"
...
python -m benchmarks.starting_node_sensitivity.starting_node_sensitivity \
    --seed "$SEED" \
    --output-dir "$OUT_DIR" \
    --mode picasso \
    --n-workers "$N_WORKERS" \
