...
| R3.5a | Justify exclusions, report removals per dataset (`:102`) | pair-accounting ladder, per dataset. **Definition frozen by T-02** ([statistics](statistics.md) §10); rungs 1–2 need T-01's re-derivation, later rungs need T-06. Fragments: `tasks/T-03-letter-R3.5a.md`, `tasks/T-02-letter-R3.5c.md` §2 | ~~T-02~~, **T-01**, T-06 | ladder table | ✓ |
...
| R3.5c | Pair dependence; describe the bootstrap; graph level (`:106`) | D2 graph-level cluster bootstrap, D3 Mantel; D15 makes it affordable. **T-02 closed 2026-08-13** — family frozen at `N_max = 197` before any p-value existed ([preregistration](preregistration.md)). Fragment drafted: `tasks/T-02-letter-R3.5c.md` | ~~T-02~~, T-06 | §3.2 | ✓ |
...
| ~~R3.7c~~ **DONE 2026-08-25** | Canonical search-space schematic (`:116`) | renderer exists: `viz/search_tree.py::canonical_search_tree_figure`. ~~**Prose already states the reviewer's exact sentence** (Remark 2.7) — only the figure is missing~~ 🔴 **It does not.** Remark 2.7 says *only* the neighbour choice is searched over; Definition 2.6 and `core/canonical.py` also search the **starting node**, and the schematic draws one subtree per start node. Measured: six start nodes, six distinct strings (lengths 9, 10, 9, 11, 10, 10), one attains the minimum. **The prose needs a one-clause fix — E13, owner T-11** | ~~T-09~~ **T-11** | §2.3 figure **done**, prose **open** | ✓ |
