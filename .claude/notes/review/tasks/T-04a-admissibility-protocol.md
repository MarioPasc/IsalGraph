...
| 2026-08-16 | **§4's triple count was wrong: `C(142,3) = 467,180`, not 470,660.** My arithmetic error, caught by track E3/E4, which implemented and asserted the correct value. Cross-checked two ways: `142·141·140/6 = 467,180`, and `C(142,2)·140 = 3·C(142,3) = 1,401,540`. The design is unaffected — it was always "all triples of the atlas" — but an acceptance criterion asked an agent to assert a false number, which is how a correct implementation could have been rejected | no; E3 ran against the correct count |
| 2026-08-16 | **§2 and §3: a per-representation size null must be computed on that representation's own pair set.** Not in the original protocol, and its absence is a real defect: on `mutagenicity` the cohort-level null is 0.754 against the upper bound while the null restricted to the 17,205 pairs `isalgraph_pruned` could encode is **0.636**, a shift of 0.117, because the 14 censored graphs are the largest and are not missing at random. A difference whose two arms are computed on different pair sets is not a comparison. Every interval this annex quotes comes from `paired_null_ci.json`, where both arms share the pair set **and** the resamples | the F5 point estimates predate it; all significance statements were computed after, on the corrected pair sets |
