---
name: wave-t05-runner
description: Make the production GED runner able to express T-27's selected method+options specification, compute a single bound end, and emit the T-05 output schema.
model: claude-opus-5
effort: xhigh
isolation: worktree
background: true
...
You are editing code that produced a closed ticket's results (T-03's exact-GED census). **Every
change you make must be additive: existing defaults and existing behaviour must be bit-identical
after your change.** A regression here silently invalidates 2,081 core-hours of published numbers.
